...
def _validate_tc_dict(tc: dict) -> dict:
    issues = []
    issues.extend(_validate_title(tc))
    issues.extend(_validate_description(tc))
    issues.extend(_validate_preconditions(tc))
    issues.extend(_validate_steps(tc))
    issues.extend(_validate_expected_outcome(tc))
    issues.extend(_validate_gwt(tc))
    issues.extend(_validate_component(tc))
...
def _has_loop_re(text, threshold=3):
    import re
...
def _validate_title(tc):
    title = tc.get("title", "").strip()
    if not title or len(title) < 10:
        return [{"field": "title", "severity": "error", "detail": "Title is empty or too short"}]
    return []


def _validate_description(tc):
...
def _validate_preconditions(tc):
...
def _validate_steps(tc):
    ...
    for i, step in enumerate(steps):
        ...
        if len(action) < 15 or any(action.startswith(f) for f in _GENERIC_FRAGMENTS):
            generic_count += 1
        if _has_loop_re(action) or _has_loop_re(result):
            loop_count += 1
...
def _validate_expected_outcome(tc):
...
def _parse_step_lines(text: str) -> list:
    """Parse plain-text numbered steps into step dicts."""
    step_lines = [ln.strip() for ln in text.split("\n") if ln.strip()]
    steps = []
...
def _register_query_tools(mcp):
    """Register read-only query tools on the MCP server."""
    ...
    @mcp.tool()
    def retrieve_examples(ac_text: str, ac_type: str, test_type: str, n: int = 2) -> dict:
        """Retrieve the n closest approved TC examples for a given AC type and test type."""
        store = _get_store()
        examples = store.retrieve_examples(ac_text=ac_text, ac_type=ac_type, test_type=test_type, n=n)
        return {
            "count": len(examples),
            "examples": [
                {
...
def _register_mutation_tools(mcp):
    """Register validation, repair, and storage tools on the MCP server."""
    ...
    def validate_test_case(tc_json: str) -> dict:
        ...
        try:
            ...
            return {
                "passed": False,
                "score": 0,
                "issues": [{"field": "json", "severity": "error", "detail": f"Invalid JSON: {e}"}],
            }
        return _validate_tc_dict(tc)

    @mcp.tool()
    def repair_test_case(tc_json: str, field: str, issue_detail: str, ac_text: str) -> dict:
        ...
        if field == "steps":
            tc[field] = _parse_step_lines(repaired_value)
        else:
        ...
        return {
            ...
            "repaired": repaired_value[:300]
            if field != "steps"
            else f"{len(tc.get('steps', []))} steps",
            "tc_updated": tc,
            "validation": validation,
        }

    @mcp.tool()
    def store_approved_tc(tc_json: str, ac_type: str, test_type: str, domain: str = "") -> dict:
        ...
        store = _get_store()
        doc_id = store.store_approved_tc(tc_dict=tc, ac_type=ac_type, test_type=test_type, domain=domain)
        stats = store.stats
        return {
            "stored": True,
            "doc_id": doc_id,
...
def cli_stats():
    ...
    store = _get_store()
    s = store.stats
    console.print("[bold]ChromaDB Stats:[/bold]")
...
def cli_validate(csv_path, tc_id):
    ...
    if result["issues"]:
        for issue in result["issues"]:
            ...
            console.print(
                f"  [{colour}]{issue['severity'].upper()}[/{colour}] {issue['field']}: {issue['detail']}"
            )
    else:
