...
@pytest.fixture()
def rm(tmp_path):
    """Create a RunManager with an isolated temp directory."""
...
class TestRunManagerCreateRun:
    ...
    def test_returns_string_id(self, rm):
        rid = rm.create_run("pipeline_a")
        assert isinstance(rid, str)
        assert len(rid) > 0

    def test_custom_run_id(self, rm):
        rid = rm.create_run("pipeline_a", run_id="my-run-1")
        assert rid == "my-run-1"

    def test_run_is_pending(self, rm):
        rid = rm.create_run("pipeline_a")
        run = rm.get_run(rid)
        assert run["status"] == "pending"

    def test_run_has_pipeline_name(self, rm):
        rid = rm.create_run("my_pipeline")
        run = rm.get_run(rid)
        assert run["pipeline_name"] == "my_pipeline"

    def test_run_has_started_at(self, rm):
        rid = rm.create_run("p")
        run = rm.get_run(rid)
        assert run["started_at"] is not None
        assert len(run["started_at"]) > 0

    def test_run_has_empty_stage_lists(self, rm):
        rid = rm.create_run("p")
        run = rm.get_run(rid)
...
class TestRunManagerGetRun:
    ...
    def test_returns_none_for_unknown(self, rm):
        assert rm.get_run("does-not-exist") is None

    def test_returns_dict_for_known(self, rm):
        rid = rm.create_run("p")
        run = rm.get_run(rid)
...
class TestRunManagerListRuns:
    ...
    def test_empty_initially(self, rm):
        assert rm.list_runs() == []

    def test_returns_all_runs(self, rm):
        rm.create_run("a")
        rm.create_run("b")
        rm.create_run("c")
        assert len(rm.list_runs()) == 3

    def test_most_recent_first(self, rm):
...
class TestRunManagerSetRunning:
    ...
    def test_status_becomes_running(self, rm):
        rid = rm.create_run("p")
        rm.set_running(rid)
...
class TestRunManagerSetCompleted:
    ...
    def test_status_becomes_completed(self, rm):
        rid = rm.create_run("p")
        rm.set_completed(rid, 5.678)
        run = rm.get_run(rid)
        assert run["status"] == "completed"

    def test_records_duration(self, rm):
        rid = rm.create_run("p")
        rm.set_completed(rid, 5.678)
        run = rm.get_run(rid)
        assert run["total_duration_seconds"] == 5.678

    def test_records_completed_at(self, rm):
        rid = rm.create_run("p")
...
class TestRunManagerSetFailed:
    ...
    def test_status_becomes_failed(self, rm):
        rid = rm.create_run("p")
        rm.set_failed(rid, "boom", 0.1)
        assert rm.get_run(rid)["status"] == "failed"

    def test_records_error_message(self, rm):
        rid = rm.create_run("p")
...
class TestRunManagerStoreArtifacts:
    ...
    def test_noop_for_unknown_run(self, rm):
        # Should not raise
...
class TestRunManagerUpdateRun:
    ...
    def test_updates_current_stage(self, rm):
        rid = rm.create_run("p")
...
class TestRunManagerPersistence:
    """Disk persistence: completed/failed runs survive restart."""

    def test_completed_run_persists(self, tmp_path):
        rm1 = RunManager(persist_dir=tmp_path)
        rid = rm1.create_run("p", run_id="persist-1")
        rm1.set_completed(rid, 1.5)

        rm2 = RunManager(persist_dir=tmp_path)
        run = rm2.get_run("persist-1")
        assert run is not None
        assert run["status"] == "completed"

    def test_failed_run_persists(self, tmp_path):
        rm1 = RunManager(persist_dir=tmp_path)
        rid = rm1.create_run("p", run_id="fail-1")
        rm1.set_failed(rid, "crashed", 0.5)

        rm2 = RunManager(persist_dir=tmp_path)
        run = rm2.get_run("fail-1")
        assert run is not None
        assert run["status"] == "failed"
        assert run["error_message"] == "crashed"

    def test_pending_run_not_persisted(self, tmp_path):
        rm1 = RunManager(persist_dir=tmp_path)
        rm1.create_run("p", run_id="pending-1")

        rm2 = RunManager(persist_dir=tmp_path)
        assert rm2.get_run("pending-1") is None
