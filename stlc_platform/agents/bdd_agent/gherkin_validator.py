...
class GherkinValidator:
    ...
    def validate(self, content: str) -> GherkinValidationResult:
        ...
        if len(features) == 0:
            ...
            return GherkinValidationResult(valid=False, errors=errors, warnings=warnings)
        if len(features) > 1:
            errors.append(
        ...
        for scenario in all_scenarios:
            self._validate_scenario(scenario, seen_names, errors, warnings)
    ...
    def _validate_scenario(
        ...
        """Validate a single scenario block for required steps and duplicates."""
        name = scenario["name"]
        ...
        # Check for When and Then
        if not self._WHEN_RE.search(block):
            errors.append(f"Scenario '{name.strip()}' is missing a 'When' step")
        if not self._THEN_RE.search(block):
            errors.append(f"Scenario '{name.strip()}' is missing a 'Then' step")

        # Check for empty step text
        self._check_empty_steps(block, name, errors)

        # Scenario Outline must have Examples
        if is_outline and not self._EXAMPLES_RE.search(block):
            errors.append(f"Scenario Outline '{name.strip()}' is missing an 'Examples:' table")

    def _check_empty_steps(self, block: str, name: str, errors: List[str]) -> None:
        """Check for empty step text in a scenario block."""
        for pattern in [self._GIVEN_RE, self._WHEN_RE, self._THEN_RE, self._AND_RE, self._BUT_RE]:
            for match in pattern.finditer(block):
                if not match.group(1).strip():
                    errors.append(f"Empty step text in scenario '{name.strip()}'")
    ...
    def _parse_outlines(self, content: str) -> List[dict]:
        ...
        return self._extract_blocks(content, r"Scenario Outline:", is_outline=True)

    def _extract_blocks(self, content: str, keyword: str, is_outline: bool) -> List[dict]:
        """
        ...
        """
        blocks = []
        pattern = re.compile(rf"^\s*{keyword}\s*(.+)", re.MULTILINE)
    ...
    def _validate_tags(self, content: str) -> List[str]:
        ...
        for line in content.split("\n"):
            ...
            if stripped.startswith("@"):
                ...
                for part in parts:
                    if part.startswith("@") and not re.match(r"^@[\w\-]+$", part):
                        errors.append(f"Malformed tag: '{part}' (tags must be @word_chars only)")
        return errors
